...
from nature_style import (
    AACR_LABELS, AACR_COLORS, NON_CANCER_COLOR,
    FIGURE_DIR, DATA_DIR, SERS_ROOT,
)

matplotlib.rcParams['font.family'] = 'DejaVu Sans'

# Load data
clinical = pd.read_csv(os.path.join(SERS_ROOT, 'data/clinical_data/standardized/all_clinical_standardized.csv'))
pan_staged = pd.read_csv(os.path.join(DATA_DIR, 'PAN_with_ajcc_stage.csv'))
pro_staged = pd.read_csv(os.path.join(DATA_DIR, 'PRO_with_staging.csv'))
crc_staged = pd.read_csv(os.path.join(DATA_DIR, 'CRC_with_ajcc_stage.csv'))
...
out_png = os.path.join(FIGURE_DIR, 'table1_demographics_v5.png')
plt.savefig(out_png, dpi=300,
            bbox_inches='tight', facecolor='white', edgecolor='none', pad_inches=0.15)
print(f"Table 1 v5 saved: {out_png}")

# Also save CSV
csv_cols = ['NOR', AACR_LABELS['PRO'], AACR_LABELS['OVA'],
            AACR_LABELS['LUN'], AACR_LABELS['PAN'], AACR_LABELS['CRC']]
rows_csv = []
for label, indent, vals, pval in table_rows:
    prefix = '  ' if indent else ''
    row_dict = {'Characteristic': prefix + label}
    for j, g_label in enumerate(csv_cols):
        row_dict[g_label] = vals[j]
    row_dict['p-Value'] = pval
    rows_csv.append(row_dict)
out_csv = os.path.join(DATA_DIR, 'table1_demographics_v5.csv')
