...
- Labels: Non-cancer, PRC, OVC, LC, PAC, CRC
"""
...
# ── Supplement smoking/drinking from raw Excel ──
# LUN 3 (SMCXD06, 100명): 흡연 여부 O/., 음주 여부 O/.
_lun3_raw = pd.read_excel(
    '/home/user/SERS-AI/data/clinical_data/4. 폐암/SMCXD06_폐암 3.xlsx',
    sheet_name='인구학적정보 및 암 관련 정보')
_lun3_smoke = _lun3_raw['흡연 여부'].map({'O': 2.0, '.': 0.0})
_lun3_drink = _lun3_raw['음주 여부'].map({'O': 2.0, '.': 0.0})

# Merge into LUN data (standardized has LUN1 30명, now add LUN3 100명)
_lun_smoke_all = pd.concat([data['LUN']['smoking_status'].dropna(), _lun3_smoke.dropna()])
_lun_drink_all = pd.concat([data['LUN']['drinking_status'].dropna(), _lun3_drink.dropna()])

# Override smoking/drinking counts per group
# display_keys = ['NC', 'PRO', 'OVC', 'LUN', 'CPAN', 'CRC']
smoke_data = {}
drink_data = {}
for g in display_keys:
    smoke_data[g] = data[g]['smoking_status'].dropna()
    drink_data[g] = data[g]['drinking_status'].dropna()
# Override LUN with supplemented data
smoke_data['LUN'] = _lun_smoke_all
drink_data['LUN'] = _lun_drink_all

smoke_known = {g: len(smoke_data[g]) for g in display_keys}
drink_known = {g: len(drink_data[g]) for g in display_keys}
...
for i, g in enumerate(display_keys):
    n_unknown = display_n[i] - smoke_known[g]
    if n_unknown > 0:
        vals.append(f"{n_unknown} ({n_unknown / display_n[i] * 100:.1f})")
    else:
        vals.append('0')
table_rows.append(('Unknown', 1, vals, ''))
...
vals = []
for i, g in enumerate(display_keys):
    n_unknown = display_n[i] - drink_known[g]
    if n_unknown > 0:
        vals.append(f"{n_unknown} ({n_unknown / display_n[i] * 100:.1f})")
    else:
        vals.append('0')
...
crc_early = 0
crc_adv = 0
for s, c in crc_staged['ajcc_stage'].value_counts().items():
    s_str = str(s).strip()
...
# Footnotes — abbreviations first, then non-cancer, then cancer
footnotes = [
    'Abbreviations: BMI, body mass index; IQR, interquartile range; yr, years.',
    'Non-cancer includes: Normal control (n=100), DM (diabetes, n=100), HTN (hypertension, n=100), DM + HTN (n=100).',
    'CRC, colorectal cancer; LC, lung cancer; OVC, ovarian cancer; PAC, pancreatic cancer; PRC, prostate cancer.',
...
# Save to AACR root structure
plt.savefig(AACR_DIR / 'figures' / 'table1_demographics_v7.png', dpi=150,
            bbox_inches='tight', facecolor='white', edgecolor='none', pad_inches=0.15)
plt.savefig(AACR_DIR / 'figures' / 'table1_demographics_v7.pdf',
            bbox_inches='tight', facecolor='white', edgecolor='none', pad_inches=0.15)
