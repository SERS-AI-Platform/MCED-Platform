...
- 🤖 **CI-enforced** — an automated check blocks merge (ruff `E,F,I,W`; scoped mypy; pytest with coverage floor; pre-commit Conventional Commits)
...
| Step 2 — subtype (output: *cancer type classification probability*) | **Cancer Type ID** | Stage 2, confidence, diagnostic confidence |
...
| Step 2 output | **Cancer type classification probability / 암종 분류 확률** | confidence, diagnostic confidence |
...
- **SSOT for the full code list, hospitals, protocols, and sample counts**: `config/config.yaml` (`folder_to_group`, `group_metadata`, `group_aliases`) + [`../.github/copilot-instructions.md`](../.github/copilot-instructions.md). **Do not hardcode counts in this doc — they drift.**
...
The **only** supported binary interpretation is whether the SSI score is **above or below 4.0**. Do **not** use "strong positive", "weak positive", or similar grading unless a separate, validated ordinal grading scheme is defined with explicit cutoffs and performance characteristics.
