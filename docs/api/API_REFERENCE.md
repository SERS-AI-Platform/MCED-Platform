...
> **Research interface only.** The API and web app in this document are not part of the SERS Clinical App and must not be used to create or interpret patient screening results. They do not implement the clinical five-replicate QC/majority-vote workflow. Clinical use is limited to `scripts/deployment/sers_clinical_webapp.py` and its packaged Windows application.

Two server applications are provided:

| Application | File | Purpose |
|---|---|---|
| REST API | `scripts/deployment/sers_api.py` | Non-clinical research JSON API |
| Web App | `scripts/deployment/sers_webapp.py` | Non-clinical research browser UI and API |
