...
MLflow는 선택 사항입니다. `pip install -e ".[mlops]"` 후 `export-dataset --mlflow-db results/mlflow.db`를 지정하면 로컬 SQLite tracking에 manifest/version과 집계 수만 기록합니다. 상세 schema, Fabric 이전, 개인정보 경계와 운영 절차는 [Clinical–Spectrum Master Data MLOps Runbook](../ml/MLOPS_MASTER_DATA.md)을 따릅니다.
...
```bash
sers data standardize                              # LEGACY 비관리형 호환 경로
sers data standardize --dry-run                    # LEGACY 미리보기만
...
`sers data standardize`는 하위 호환성을 위해서만 유지되는 비관리형 predecessor이며 실행 시 `DEPRECATED` 경고를 출력합니다. 이 경로의 `CPAN → PAN`, `BLA → BLC`, 파일/Excel 행 순서 기반 결과는 governed lineage의 임상 source, identity matching, label evidence 또는 dataset manifest 입력으로 사용할 수 없습니다. 관리형 canonical 경로는 `sers data ingest-clinical-registry`이며, source-file → protocol 계약은 `src/sers/master_data/clinical_inventory.py`를 따릅니다.
