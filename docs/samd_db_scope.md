...
- QC Pass가 몇 개인가
- 어떤 모델 버전으로 분석했는가
- SSI, Majority Vote, Cancer Probability, Confidence가 어떻게 나왔는가
- 어떤 보고서가 생성되었는가
...
| 분석 결과 표시 | SSI, 확률, Confidence 표시 | analysis_results | ANALYSIS_RUN |
| 보고서 생성/조회 | PDF/CSV 생성 및 조회 | reports | REPORT_CREATE / REPORT_VIEW |

내부 IFU 문서에서는 AECD 소프트웨어 사용을 위해 사용자 인증이 필요하고, 관리자/의사/검사실 기사 역할에 따라 접근 권한이 차등 적용된다고 정리되어 있으며, 환자 ID, 나이, 성별, BMI와 선택적 임상 메타데이터 입력 후 SERS 스펙트럼 CSV 또는 TXT 파일 5개를 업로드하는 흐름도 설명되어 있다.
...
| 검사실 기사 | 스펙트럼 업로드, QC 실행, AI 분석, 보고서 생성 | 사용자 권한 변경 불가 |
| 진검과 의사 | 환자 정보 입력/조회, 결과지 조회 | Raw 파일 삭제/수정 불가 |

이 구조는 내부 IFU의 "관리자/의사/검사실 기사 역할에 따라 접근 권한 차등 적용"이라는 방향과 맞는다.
...
내부 IFU에서는 업로드된 5개 스펙트럼을 자동으로 신호강도, 스파이크 잡음, 포화 여부, replicate 간 상관도 기준으로 검사하고, QC 결과를 스펙트럼별 Pass/Fail 및 실패 사유별 건수로 표시한다고 되어 있다. 또한 5회 측정 중 QC Pass가 3회 이상이면 검사 유효, 3회 미만이면 검사 무효이며 최종 결과를 해석하지 않는다고 정리되어 있다.
...
처리:
- 전처리
- uSERS-Net inference (StackingPredictor: 10개 base model + ElasticNet meta-learner)
- 단일 고정 threshold 적용 (balanced / Youden's J — 3-preset 선택 UI/코드 없음)
- majority vote (replicate 과반수 기준 — 평균확률 기준 아님)
- risk stratification (SSI 3단계: LOW / MODERATE / HIGH)
- confidence 산출 (암종 분류 top-1 확률·gap 기반 등급)
- explainability — 설계 문서만 존재, 코드 미구현 (개발 예정)

출력:
- SSI (0~10, threshold가 항상 4.0에 매핑되는 구간별 선형 변환)
- Positive/Negative (SSI ≥ 4.0 / < 4.0 && majority vote 과반수 기준)
- Cancer probability
- Cancer type probability
- Confidence (등급: 상/중/하 — 숫자 점수 아님)
- Top peaks / contributed peaks — 개발 예정 (미구현)
```

내부 제품 설명서에서는 모델을 uSERS-Net 자체 개발 AI 분류 모델로 정의하고, 입력은 라만 분광기 측정 CSV 파일, 출력은 양/음성 및 추정 암종과 PDF 임상 보고서 생성으로 정리되어 있다. IFU에서는 AECD 암 선별 판정 기준으로 환자 단위 암 확률 SSI를 이용하고, SSI ≥ 4.0이면 양성, SSI < 4.0이면 음성이라는 내부 고정 판정 정책이 정리되어 있으며, 양성으로 판정된 경우에만 암종별 확률과 Confidence를 보조 정보로 표시한다.

> **[2026-07-02 코드 검증 정정 사항]**
>
> - **Confidence 정의**: "1순위 암종 softmax 확률 × 10"이 아니다. 실제로는 `scripts/deployment/clinical_decision.py`의 `type_confidence()`가 산출하는 3단계 등급이다 — top_prob≥0.50 또는 gap≥0.20 → **high**, top_prob≥0.30 또는 gap≥0.10 → **medium**, 그 외 → **low** (gap = top-1 확률 − top-2 확률). IFU/사양서에는 숫자(×10) 대신 이 등급(상/중/하)으로 표기해야 실제 출력과 일치한다.
...
> - **Risk Stratification**: STK-V2 nested 5-fold OOF CV(n=1628)로 검증한 3단계를 사용한다 — **LOW** [SSI 0-1), **MODERATE** [SSI 1-4), **HIGH** [SSI 4-10]. 관측 암 비율: LOW 1.8%(n=388), MODERATE 46.0%(n=50, 95% CI 32.7-59.7%), HIGH 98.2%(n=1190). MODERATE는 SSI<4.0(Negative 판정)에 속하지만 위험도가 유의하게 높은 구간으로, "음성이지만 안심할 수 없는" 환자를 구분하기 위한 것이다. 검증 스크립트: `scripts/analysis/validate_ssi_risk_bands.py`. 원래 검토했던 5단계(균등구간)는 중간 구간 표본이 너무 적어(n=17-25) 채택하지 않았다.
> - **Explainability(Top peaks / contributed peaks)**: 출력 항목으로 나열돼 있으나 **현재 미구현**이다. 존재하는 것은 AACR 논문용 cohort-level SHAP(암종 평균 스펙트럼 vs 전체 평균 스펙트럼)뿐이며, 환자 1건 단위 기여 피크 계산은 설계 문서(`docs/explainability_design_peak_attribution.md`)만 작성되어 있다. IFU에 "제공 기능"으로 표기하려면 구현이 선행되어야 한다.
...
- analysis_run_id
- ssi_score
- binary_result          -- POSITIVE / NEGATIVE (SSI 4.0 cutoff)
- majority_vote_result   -- "n_detected/n_total", 과반수 기준
- risk_level             -- [추가] LOW / MODERATE / HIGH
- confidence_level       -- [변경] confidence_score(숫자) 대신 등급(high/medium/low) — 실제 산출 방식과 일치
- top_cancer_type
...
IF QC_STATUS = VALID AND SSI >= 4.0:
    POSITIVE 표시
    암종별 확률 표시
    Confidence 표시
    정밀검사 권고 문구 표시
...
- REQ-AUTH-002: 시스템은 관리자, 검사실 기사, 진검과 의사 역할을 구분해야 한다.
...
- REQ-QC-003: 시스템은 QC Pass 개수가 3개 이상이면 Valid, 3개 미만이면 Invalid로 판정해야 한다.
...
- REQ-AI-003: 시스템은 SSI, 양성/음성, 암종별 확률, Confidence(등급)를 산출해야 한다.
- REQ-AI-004: 시스템은 분석 실행자, 실행 시간, 입력 데이터 ID, 모델 버전을 저장해야 한다.
- REQ-AI-005: 분석 실행은 audit log에 기록되어야 한다.
- REQ-AI-006: [추가] 시스템은 SSI 기반 3단계 Risk Stratification(LOW/MODERATE/HIGH)을 산출하고 결과에 함께 저장해야 한다.
- REQ-AI-007: [추가, 개발 예정] Explainability(기여 피크)는 구현 완료 전까지 IFU/사양서에 "제공 기능"으로 표기하지 않는다.

**F. 결과/보고서 모듈**
- REQ-REP-001: 시스템은 분석 결과 기반 PDF/CSV 보고서를 생성해야 한다.
- REQ-REP-002: 보고서에는 환자 ID, QC 상태, SSI, 결과, 모델 버전, 생성 시간이 포함되어야 한다.
...
4. QC
   ...
   - QC Pass ≥ 3: Valid / <3: Invalid
   - DB: qc_runs, qc_results
   - Audit: QC 실행/결과

5. AI 분석
   - uSERS-Net inference, SSI, probability, confidence(등급), risk stratification(3단계)
