...
try:
    from sers.models._legacy.resnet_v1.model import ModelConfig, build_model
except ImportError:
...
if _KO_FONT.exists():
    _fm.fontManager.addfont(str(_KO_FONT))
...
# Load group colors from config
_config_path = PROJECT_ROOT / "config" / "config.yaml"
if _config_path.exists():
    with open(_config_path) as _f:
        _display = yaml.safe_load(_f).get("display", {})
