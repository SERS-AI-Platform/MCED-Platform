...
Usage:
    python models/build_production_stacking.py
"""
...
def main():
    ...
    out_dir = PROJECT_ROOT / "artifacts" / "usersnet" / "v1.0.0"
    ...
    # Screening: highest threshold where sensitivity (TPR) >= 0.95
    # → catches ≥95% of cancers with as few false positives as possible
    sens_mask = tpr >= 0.95
    if sens_mask.any():
        screening_thresh = float(thresholds[sens_mask].max())
    else:
        screening_thresh = float(thresholds[np.argmin(np.abs(tpr - 0.95))])

    # Confirmatory: lowest threshold where specificity (1-FPR) >= 0.95
    # → only flags high-confidence cases (few false positives)
    spec_mask = (1 - fpr) >= 0.95
    if spec_mask.any():
        confirmatory_thresh = float(thresholds[spec_mask].min())
    else:
        confirmatory_thresh = float(thresholds[np.argmin(np.abs((1 - fpr) - 0.95))])

    # Sanity: screening ≤ balanced ≤ confirmatory
    if screening_thresh > balanced_thresh:
        logger.warning(f"Screening ({screening_thresh:.4f}) > balanced ({balanced_thresh:.4f}), clamping")
        screening_thresh = balanced_thresh * 0.9
    if confirmatory_thresh < balanced_thresh:
        logger.warning(f"Confirmatory ({confirmatory_thresh:.4f}) < balanced ({balanced_thresh:.4f}), clamping")
        confirmatory_thresh = balanced_thresh * 1.1
