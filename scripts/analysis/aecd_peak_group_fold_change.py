...
def write_readme(table: pd.DataFrame, metadata: dict, path: Path) -> None:
    ...
    lines += [
        ...
        "동일 파수의 값을 측정해 비교한다. 다만 이 컬럼을 생물학적 차이로 읽으면 안 된다 — "
        "2026-09-02 정상군에서 subject 1명(BNOR_110)을 제외했을 때 registry가 9개에서 11개로 "
        "바뀌고 4개 피크의 `detected_in_labels`가 뒤집혔다. 같은 이유로 CI가 1을 제외하는지 "
        "여부도 정상군 subject 1명에 뒤집힐 수 있다.",
