#!/usr/bin/env python3
"""
Experiment 5: Full Stacking / Meta-Learner Optimization
========================================================
...
Usage:
    python scripts/analysis/weekend_experiments/stacking_optimization.py
    python scripts/analysis/weekend_experiments/stacking_optimization.py --dry-run
"""
...
FIG_OUTPUT_DIR = FIG_DIR / "weekend" / "stacking_optimization"
...
def run_val_group_inference(args):
    ...
    # Load from saved oof_predictions.npz
    oof_path = OUTPUT_DIR / "oof_predictions.npz"
    if not oof_path.exists():
        logger.error(f"  OOF predictions not found: {oof_path}")
        logger.error("  Run stacking_optimization.py first to generate OOF predictions.")
        return 1
