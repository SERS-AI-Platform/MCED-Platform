...
def _save_assignment_table(df: pd.DataFrame) -> None:
    ...
    table.to_csv(os.path.join(out_dir, "fig2b_cancer_type_peak_table.csv"), index=False)
...
def main() -> None:
    ...
    other_stats = {
        group: _mean_stats(X_3ch, cancer_pool & (groups != group))
        for group in stats
    }
    ...
    for i, (group, ax) in enumerate(zip(order, axes)):
        ...
        for _, row in shade_peaks.iterrows():
            ...
            ax.text(
                peak, label_y + label_level * 0.13 * y_range,
                f"{peak}",
                ha="center", va="bottom",
                fontsize=6.8,
                fontweight="bold",
    ...
    fig.suptitle("Cancer Type Discrimination Peak Feature Importance", fontsize=9.5, fontweight="bold", y=0.995)
