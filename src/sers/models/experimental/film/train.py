"""
FiLM Fusion Training — Clinical-conditioned ResNet18-1D

Usage:
    python models/film/train.py
    python models/film/train.py --aggregate medoid --n-splits 5
    python models/film/train.py --film-hidden 128 --epochs 200

Outputs (results/training/<experiment>/film/):
...
from sers.models._legacy.resnet_v1.model import ModelConfig, TwoStageLoss, model_summary
...
from sers.models.experimental.film.model import FiLMConfig, FiLMCancerDetector, build_film_model, film_model_summary
...
# Cross-Validator
# ---------------------------------------------------------------------------
class FiLMCrossValidator(BaseCrossValidator):

    def __init__(self, X, binary_labels, cancer_type_labels, sample_ids, groups_arr,
                 clinical_features, config, film_config, device, runtime,
    ...
    def _run_fold(self, fold_idx, train_idx, val_idx):
        ...
        model = build_film_model(self.config, self.film_config).to(self.device)
        ...
        torch.save(
            {
                ...
                "model_name": "film_resnet18",
                "config": self.config.__dict__,
                "film_config": self.film_config.__dict__,
                "train_idx": train_idx,
                "val_idx": val_idx,
            },
...
# MLflow
# ---------------------------------------------------------------------------
def log_mlflow_film(results, config, film_config, args):
    try:
        import mlflow
        mlflow.set_tracking_uri(f"sqlite:///{PROJECT_ROOT / 'mlflow.db'}")
        mlflow.set_experiment("sers-fusion-film")
        with mlflow.start_run(run_name=f"film_{args.aggregate}_{args.n_splits}fold"):
            mlflow.log_params({
                "fusion_type": "film",
                "model": "FiLM-ResNet18-1D",
                ...
                "n_clinical": film_config.n_clinical,
                "film_hidden": film_config.film_hidden,
            })
...
# ---------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="FiLM Fusion — SERS + Clinical Training")
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(experiment_dir / "train_film.log", mode="w", encoding="utf-8"),
        ],
    )

    logger.info("=" * 64)
    logger.info("  FiLM Fusion — SERS + Clinical Training")
    logger.info("=" * 64)
    ...
    # Step 3: Load clinical data and merge
    ...
    # Step 6: Merge clinical features (after aggregation, aligned with X)
    ...
    # FiLM config
    film_config = FiLMConfig(n_clinical=args.n_clinical, film_hidden=args.film_hidden)

    # Model summary
    model = build_film_model(mc, film_config)
    logger.info(f"\n{film_model_summary(model)}")
    del model

    # Step 7: Cross-validation
    run_out_dir, version_name = resolve_run_output_dir(experiment_dir, "film_resnet18", args.version)
    ...
    logger.info(f"\n[Step 7] {args.n_splits}-fold CV for FiLM-ResNet18-1D ({version_name})...")
    cv_runner = FiLMCrossValidator(
        X, bl, ctl, sample_ids, groups_arr,
        clinical_features, mc, film_config, device, runtime,
    ...
    model_display_name = "FiLM-ResNet18-1D"
    model_params = {
    ...
    summary = build_training_summary(
        ...
        n_samples=len(X),
        n_features=X.shape[-1],  # spectral length (last dim for multichannel)
    ...
    if not args.no_mlflow:
        log_mlflow_film(results, mc, film_config, args)
