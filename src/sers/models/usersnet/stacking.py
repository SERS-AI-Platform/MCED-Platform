...
def infer_sex_from_groups(groups_arr):
    """Infer sex from cancer group labels (PRO=male=1.0, OVA=female=0.0, else=NaN)."""
    sex = np.full(len(groups_arr), np.nan)
...
def apply_sex_constraint(type_probs, cancer_types, sex_arr):
    ...
    for i, name in enumerate(cancer_types):
        if name == "OVA":
            ova_idx = i
        elif name == "PRO":
            pro_idx = i
