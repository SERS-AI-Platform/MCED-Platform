"""LEGACY NON-GOVERNED AACR staging reproduction workflow.

This module contains historical row-order linkage and is forbidden as governed
lineage input. It fails closed unless invoked with
``--allow-legacy-row-order-linkage`` for historical reproduction.
...
from .reingest_staging_rules import (
...
pro_stage_df['grade_group'] = pro_stage_df['gleason_raw'].apply(gleason_to_grade_group)
